...
export async function switchLeafOrderJumpUp(params: any, id: number, filteringStatusGroup: string[]) {
  ...
  if (index === 0) return;
  const upId = filteredLeaves[index - 1].id;
  await switchLeafOrder(params, id, upId);
}
